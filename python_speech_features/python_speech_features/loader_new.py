...
# (rate, width, sig) = wavio.readwav(tmp_file_path)
# sig = sig.ravel()
# signal = torch.FloatTensor(sig)
# #
# # # (514, 13)
# tmp_mfcc = mfcc(signal,samplerate = SAMPLE_RATE, winlen=0.030, winstep=0.01,
#                  nfilt=40, nfft=N_FFT)
# tmp_mfcc.shape
​
# tmp_mfcc = mfcc(signal,samplerate = SAMPLE_RATE, winlen=0.030, winstep=0.01, numcep=13,
...
def get_mfcc_feature(filepath):
    ...
    sig = sig.ravel()
    signal = torch.FloatTensor(sig)
    feat = mfcc(signal, samplerate = SAMPLE_RATE, winlen=0.030, winstep=0.01, numcep=13,
                 nfilt=40, nfft=N_FFT, lowfreq=0,highfreq=None,preemph=0.97,
                ceplifter=22, appendEnergy=True)
...
​
    mel_spectrogram = librosa.feature.melspectrogram(sig, n_mels = 40, n_fft = 512, hop_length = 128)
​
    return mel_spectrogram
​
## plot 2D spectrogram
# import matplotlib.pyplot as plt
# from librosa import displa
#
